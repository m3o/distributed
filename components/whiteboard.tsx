...
export default class Whiteboard extends React.Component<Props, State> {
  ...
  async setupWebsocket() {
    const rsp = await fetch(`/api/whiteboard/${this.props.id}`, { method: 'GET' })
    ...
    ws.onopen = () => {
      ...
      this.setupCanvas();
      ws.send(JSON.stringify({ token: data.websocket.token, topic: data.websocket.topic }))
    }
    ...
    ws.onmessage = ({ data }) => {
      ...
      if(this.state.drawnIDs.includes(payload.id)) {
        console.log(`Skipping whiteboard event ${payload.id}, message sent locally`);
        return
      }
  ...
  sendEvent(data: { x: number, y: number, status: 'move' | 'draw', id: string }) {
    this.setState({ drawnIDs: [...this.state.drawnIDs, data.id] })
    fetch(`/api/whiteboard/${this.props.id}`, { method: 'POST', body: JSON.stringify(data) })
  }
  ...
  render(): JSX.Element {
    ...
    return <canvas height={400} width={400} ref={this.canvas} className={styles.whiteboard} />
  }
}
